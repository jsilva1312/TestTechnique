...
int main(int argc, char *argv[])
{
    ...
    if(argc != 2)
    {
        fprintf(stderr, "Bad usage : udp_reader <port>");
        exit(EXIT_FAILURE);
    }
    else
    {
        if(strlen(argv[1]) > PORT_ARRAY_SIZE)
        {
            fprintf(stderr, "Bad usage : Port must be less than 99999 ");
            exit(EXIT_FAILURE);
        }
        strncpy(listenPort, argv[1], strlen(argv[1]) );
